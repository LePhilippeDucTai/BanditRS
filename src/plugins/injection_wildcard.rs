...
pub fn linux_commands_wildcard_injection(ctx: &Context<'_, '_>, cfg: &PluginConfigs) -> PluginResult {
    let shell_cfg = &cfg.shell_injection;
    if matches!(shell_cfg.shell, ListOpt::Missing) || matches!(shell_cfg.subprocess, ListOpt::Missing) {
        return Ok(None);
    }
    let qual = ctx.call_function_name_qual().unwrap_or("");
    let in_shell = shell_cfg.shell.items("shell")?.iter().any(|s| s == qual);
    let in_subprocess_with_shell =
        shell_cfg.subprocess.items("subprocess")?.iter().any(|s| s == qual) && ctx.check_call_arg_is("shell", &PyValue::str("True"))? == Some(true);
    ...
    let argument_string = match ctx.get_call_arg_at_position(0)? {
        Some(PyValue::List(items)) => items.iter().map(|i| format!(" {}", i.py_str())).collect::<String>(),
    ...
    if !argument_string.is_empty() {
        for f in VULNERABLE_FUNCS {
            if argument_string.contains(f) && argument_string.contains('*') {
                return Ok(Some(
                    IssueDraft::new(Rank::High, Rank::Medium, Cwe::IMPROPER_WILDCARD_NEUTRALIZATION, format!("Possible wildcard injection in call: {qual}"))
