...
fn report(value: &str, lineno: Option<u32>) -> IssueDraft {
    IssueDraft::new(Rank::Low, Rank::Medium, Cwe::HARD_CODED_PASSWORD, format!("Possible hardcoded password: '{value}'")).with_lineno(lineno)
}

/// `hardcoded_password_string` (B105).
pub fn hardcoded_password_string(ctx: &Context<'_, '_>, _cfg: &PluginConfigs) -> PluginResult {
    let Some(node) = ctx.node else { return Ok(None) };
    let Some(parent) = ctx.parent() else { return Ok(None) };
    match parent {
        ...
        VNode::Expr(Expr::Dict(d)) => {
            if let Some(node_expr) = node.as_expr() {
                if let Some(pos) = d.items.iter().position(|it| it.key.as_ref().is_some_and(|k| std::ptr::eq(k, node_expr))) {
                    if ctx.string_val().is_some_and(|s| RE_CANDIDATES.is_match(s)) {
                        if let Expr::StringLiteral(_) | Expr::NumberLiteral(_) | Expr::BooleanLiteral(_) | Expr::NoneLiteral(_) | Expr::BytesLiteral(_) =
                            &d.items[pos].value
                        {
        ...
        VNode::Expr(Expr::Subscript(_)) => {
            if ctx.string_val().is_some_and(|s| RE_CANDIDATES.is_match(s)) {
                if let Some(VNode::Stmt(Stmt::Assign(a))) = ctx.ancestor(2) {
                    if let Expr::StringLiteral(v) = &*a.value {
        ...
        VNode::Expr(Expr::Compare(c)) => {
            ...
            if candidate.is_some_and(|n| RE_CANDIDATES.is_match(n)) {
                if let Some(Expr::StringLiteral(v)) = c.comparators.first() {
                    return Ok(Some(report(v.value.to_str(), None)));
                }
            }
        }
...
pub fn hardcoded_password_funcarg(ctx: &Context<'_, '_>, _cfg: &PluginConfigs) -> PluginResult {
    ...
    for kw in &call.arguments.keywords {
        ...
        if let Expr::StringLiteral(s) = &kw.value {
            if RE_CANDIDATES.is_match(arg.as_str()) {
...
pub fn hardcoded_password_default(ctx: &Context<'_, '_>, _cfg: &PluginConfigs) -> PluginResult {
    let Some(f) = ctx.function else { return Ok(None) };
    for p in f.parameters.args.iter() {
        ...
        if let Expr::StringLiteral(s) = default {
            if RE_CANDIDATES.is_match(p.name().as_str()) {
