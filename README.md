...
`scripts/check.sh` reproduces exactly the three jobs of the GitHub Actions workflow, kept but inert in
`.github/workflows/ci.yml.disabled`. Exit `0` = the equivalent of a green CI.

### Checking parity yourself

```bash
cargo test --test golden                      # replays the golden corpus — no Python required
scripts/diff_against_python.sh examples       # differential over the fixtures
scripts/diff_against_python.sh --stdlib       # differential over 672 stdlib files
scripts/gen_golden.sh                         # regenerate the golden corpus
```

The differential expects a reference Python `bandit`; by default `/home/user/.pyenv-bandit/bin/bandit`,
overridable through the `PY_BANDIT` variable.
