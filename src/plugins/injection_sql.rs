...
static SIMPLE_SQL_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?is)(select\s.*from\s|delete\s+from\s|insert\s+into\s.*values[\s(]|update\s.*set\s)").unwrap()
});

/// Collect the string-constant leaves of a `BinOp` chain (`_get`), skipping
/// the subtree rooted at `stop` (identity comparison).
fn collect_binop_strings<'a>(node: &'a Expr, stop: *const Expr, bits: &mut Vec<String>) {
...
fn push_leaf<'a>(e: &'a Expr, stop: *const Expr, bits: &mut Vec<String>) {
...
fn evaluate_ast(ctx: &Context<'_, '_>) -> (bool, String, bool) {
    ...
    match ctx.node {
        Some(VNode::JoinedConst(view, i)) => {
            let const_indices: Vec<u32> =
                view.values.iter().enumerate().filter(|(_, p)| p.is_constant()).map(|(idx, _)| idx as u32).collect();
            if const_indices.first() == Some(&i) {
                statement = const_indices.iter().filter_map(|&idx| view.values[idx as usize].constant_text()).collect::<Vec<_>>().join("");
        ...
        _ => match ctx.parent() {
            Some(VNode::Expr(Expr::BinOp(_))) => {
                ...
                wrapper = if idx > 0 { Some(ancestors[idx - 1]) } else { None };
                let VNode::Expr(stop_expr) = ancestors[len - 1] else { unreachable!() };
                let VNode::Expr(root_expr) = ancestors[idx] else { unreachable!() };
            ...
            Some(VNode::Expr(Expr::Attribute(a))) if a.attr.as_str() == "format" || a.attr.as_str() == "replace" => {
    ...
    let execute_call = match wrapper {
        Some(VNode::Expr(Expr::Call(call))) => matches!(called_name(call), "execute" | "executemany"),
        _ => false,
    };
...
pub fn hardcoded_sql_expressions(ctx: &Context<'_, '_>, _cfg: &PluginConfigs) -> PluginResult {
    ...
    if SIMPLE_SQL_RE.is_match(&statement) {
        let confidence = if execute_call && !str_replace { Rank::Medium } else { Rank::Low };
