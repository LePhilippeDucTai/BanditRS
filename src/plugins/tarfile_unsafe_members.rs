...
fn get_members_issue(arg: &Expr) -> PluginResult {
    let (kind, repr): (&str, String) = match arg {
        Expr::Call(call) => match &*call.func {
            ...
            _ => {
                return Err(PyErr::attribute_error(
    ...
    if kind == "Function" {
        ...
        Ok(Some(IssueDraft::new(
            ...
            Cwe::PATH_TRAVERSAL,
            format!(
                "Found tarfile.extractall(members=?) but couldn't identify the type of members. Check if the members were properly validated {members_repr})."
            ),
        )))
    }
}
...
pub fn tarfile_unsafe_members(ctx: &Context<'_, '_>, _cfg: &PluginConfigs) -> PluginResult {
    if !ctx.is_module_imported_exact("tarfile")
        || !ctx
            .call_function_name()
            .unwrap_or("")
            .contains("extractall")
    ...
    if let Some(kw) = call
        ...
        .keywords
        .iter()
        .find(|k| k.arg.as_ref().is_some_and(|a| a.as_str() == "filter"))
        && is_filter_data(&kw.value)
    {
        return Ok(None);
    }
    if let Some(kw) = call
        ...
        .keywords
        .iter()
        .find(|k| k.arg.as_ref().is_some_and(|a| a.as_str() == "members"))
    {
